...
export default function CompanionsTab({ character, onUpdate, onHideTab }) {
  ...
  const addCompanion = () => {
    const newCompanion = {
      id: generateId('companion'), name: '', type: 'familiar', form: '', ac: 10, maxHp: 1, currentHp: 1,
    ...
    <div>
                    ...
                    <div className="grid grid-cols-6 gap-2">
                      {['str', 'dex', 'con', 'int', 'wis', 'cha'].map(stat => (
                        <div key={stat} className="text-center">
                          ...
                          <div className="text-xs text-stone-400">{getMod(companion[stat])}</div>
                        </div>
                      ))}
                    </div>
                  </div>
      ...
      {(character.companions || []).length === 0 && (
        <div className="text-center text-stone-500 py-8 border border-dashed border-stone-700 rounded-lg">
          <p>No companions yet. Add familiars, animal companions, or mounts.</p>
          {onHideTab && (
            <button onClick={onHideTab} className="mt-2 text-stone-400 hover:text-stone-200 text-xs underline">
              Hide this tab
            </button>
          )}
        </div>
